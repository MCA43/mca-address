...
use Mca\Address\Support\AddressDataPaths;
...
class TurkeyNeighborhoodImporter
{
    ...
    public function resolveDataPath(?string $source = null): string
    {
        ...
        $path = AddressDataPaths::turkeyNeighborhoodsJson();

        if (! is_file($path)) {
            throw new RuntimeException("Paket mahalle verisi bulunamadı: {$path}");
        }
